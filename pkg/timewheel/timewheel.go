...
// New 创建时间轮
func New(intervalSeconds int64, slotNum int, job Job) *TimeWheel {
	if intervalSeconds <= 0 || slotNum <= 0 || job == nil {
		return nil
	}
	tw := &TimeWheel{
...
func (tw *TimeWheel) AddTimer(delay time.Duration, key any, data any) {
	if delay < 0 {
		return
	}
